...
smc_nopt_v2 <- function(iter, max_time=Inf, sigma0, proportion_obs0, death_rate, ptree, day = 0, noisy_prevalence, n_particles=NULL, ess_threshold = n_particles/2, resampling_scheme = "systematic", backward_sim = TRUE, print=F) {
  ...
  if (is.null(n_particles)) {
    ...
    I <- 1000
    #allow to continue if acceptance rate is above a_threshold
    #initially 0
    a_init <- 0
    a_threshold <- 0.05

    #continue as long as acceptance rate is too low and we haven't hit particle cap
    while (a_init < a_threshold & n_particles < 2000) {
      ...
      #keep adding 100 until desired acceptance rate is hit
