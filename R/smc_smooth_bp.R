...
#'
#' @examples
#' smc_smooth_bp(iter = 100000, birth_rate_0 = 0.1, prevalence_0 = prev_0, proportion_obs_0 = 0.2, death_rate = 0.1, ptree = sample_tree, noisy_prevalence = noisy_prev, n_particles = 100)
smc_smooth_bp <- function(iter, birth_rate_0, max_birth_rate=100, prevalence_0, proportion_obs_0, death_rate, ptree, noisy_prevalence, n_particles, print=F, plot=F) {
  ...
  for (i in 1:iter) {
    ...
    f_hat_new <- sir_smooth(n_particles = n_particles, birth_rate = b_new, death_rate = death_rate, proportion_obs = p_new, noisy_prevalence = noisy_prevalence, genetic_data = genetic_data, plot = plot)
