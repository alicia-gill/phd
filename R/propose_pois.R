...
propose_pois <- function(noisy_prevalence, proportion_obs, birth_rate, death_rate, ptree, ...) {
  ...
  prev_new[-1] <- smooth_prev[-1,2] +
                  extraDistr::rtpois(n_days, lambda = lambda, a = noisy_prevalence[-1,2] + round(lambda) - smooth_prev[-1,2]) -
                  round(lambda)
  ...
  q <- sum(extraDistr::dtpois(prev_new[-1,2] - smooth_prev[-1,2] + round(lambda), lambda = lambda, a = noisy_prevalence[-1,2] + round(lambda) - smooth_prev[-1,2], log = TRUE))
